...
local function dumpValues(path, modules_table, colors_table, sounds_table)
	...
	if f then
		...
		if modulesfile_data:find('<Include file="Options\\options.xml"/>') then
			-- remove old files
			local oldOptionsFile = path .. "Options.xml"
			local oldSoundFile = path .. "Sounds.lua"
			local oldColorsFile = path .. "Colors.lua"
			os.remove(oldOptionsFile)
			os.remove(oldSoundFile)
			os.remove(oldColorsFile)
			-- update path to new file
			modulesfile_data = modulesfile_data:gsub('<Include file="Options\\options.xml"/>', '<Script file="Options\\Options.lua"/>')
