...
L.fontSizeDesc = "使用滑条或在框内输入数值可调整字体尺寸，请不要超过最大数值（需有效数值）。"
L.disabled = "禁用"
...
L.additionalSizeDesc = "移动此滑条可增加显示的尺寸，或在数字框内输入数值，请不要超过最大数值（需有效数值）。"
...
L.testNameplateIconBtn_desc = "创建一个测试图标，供您在目标姓名板上测试当前显示设置。"
L.stopTestNameplateIconBtn = "停止测试图标"
L.stopTestNameplateIconBtn_desc = "停止姓名板上的所有测试图标。"
...
L.keepAspectRatioDesc = "保持图标的宽高比为1:1，而不是将其拉伸以适应框体大小。"
L.iconColor = "图标颜色"
