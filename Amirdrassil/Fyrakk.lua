...
function mod:OnEngage()
	...
	self:Bar(419506, 13, CL.count:format(self:SpellName(419506), firestormCount)) -- Firestorm
	self:Bar(417455, 42, CL.count:format(self:SpellName(417455), dreamRendCount)) -- Dream Rend
...
function mod:Firestorm(args)
	...
	self:Bar(args.spellId, 53.5, CL.count:format(args.spellName, firestormCount))
end
...
function mod:DreamRend(args)
	...
	self:Bar(args.spellId, 53.6, CL.count:format(args.spellName, dreamRendCount))
end
...
function mod:InfernalMawApplied(args)
	-- XXX not sure what to do with this, 4 is the amount of applications between mechanics
	local amount = args.amount or 1
	if amount % 4 == 0 then
