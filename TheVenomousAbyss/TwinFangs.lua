...
mod:SetAuraData({ -- TODO
	...
	{1310096, soundOnApplied = "none", soundOnAppliedDose = "none", duration = 8}, -- Feasted
...
function mod:OtherEvents(_, eventInfo)
	...
	local duration = eventInfo.duration
	local durationRounded = self:RoundNumber(self:RoundNumber(duration, 1), 0)

	if durationRounded == (self:Easy() and 163 or 144) then
