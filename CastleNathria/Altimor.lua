...
function mod:OnBossEnable()
	...
	self:Log("SPELL_AURA_APPLIED", "HuntsmansMarkApplied", 335111, 335112, 335113)
...
function mod:Sinseeker(args)
	-- self:Message2(args.spellId, "red", CL.count:format(args.spellName, sinseekerCount)) -- XXX Do we want to warn faster, without targets?
...
do
	...
	function mod:HuntsmansMarkApplied(args)
		...
		if self:Me(args.destGUID) then
			self:Say(335114, CL.count_rticon:format(self:SpellName(335114), playerIconsCount, playerIconsCount))
			self:PlaySound(335114, "warning")
			self:Flash(335114)
		end
		if self:GetOption(sinseekerMarker) then
			SetRaidTarget(335114, playerIconsCount)
		end
